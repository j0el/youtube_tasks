...
def detect_crossovers(df):
    ...
    for i in range(1, len(df)):
        if (
            df["SMA_5"].iloc[i] > df["SMA_13"].iloc[i]
            and df["SMA_8"].iloc[i] > df["SMA_13"].iloc[i]
            and df["SMA_5"].iloc[i - 1] <= df["SMA_13"].iloc[i - 1]
        ):
            buy_signals.append((df.index[i], df["Close"].iloc[i]))
        elif (
            df["SMA_8"].iloc[i] < df["SMA_13"].iloc[i]
            and df["SMA_5"].iloc[i] < df["SMA_8"].iloc[i]
            and df["SMA_8"].iloc[i - 1] >= df["SMA_13"].iloc[i - 1]
        ):
