...
def load_stock_symbols():
    try:
        ...
        st.error(
            "The 'stocks.txt' file was not found. Please make sure it is in the same directory."
        )
        return []
...
def calculate_indicators(data):
    ...
    # Bollinger Bands
    data["Upper Band"] = data["Close"].rolling(20).mean() + (
        data["Close"].rolling(20).std() * 2
    )
    data["Lower Band"] = data["Close"].rolling(20).mean() - (
        data["Close"].rolling(20).std() * 2
    )
    return data
...
if not stock_symbols:
    ...
    if data is not None:
        ...
        def color_row(row, prev_row):
            ...
            colors["MACD"] = (
                "color: green" if row["MACD"] > row["Signal Line"] else "color: red"
            )
            colors["SMA50"] = (
                "color: green" if row["SMA50"] > row["SMA200"] else "color: red"
            )
        ...
        for i in range(len(df_metrics)):
            if i == 0:
                ...
                color_mapping.append(
                    color_row(df_metrics.iloc[i], df_metrics.iloc[i - 1])
                )
