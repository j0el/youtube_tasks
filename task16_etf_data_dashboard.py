...
for etf in etf_list:
    try:
        ...
        ratio_to_52W_Max = round(
            (etf_metrics["Current Price"] / etf_metrics["52W Max"]) * 100, 2
        )
        ratio_to_52W_Min = round(
            (etf_metrics["Current Price"] / etf_metrics["52W Min"]) * 100, 2
        )
        ...
        fig1.add_trace(
            go.Scatter(
                x=hist_data.index, y=hist_data["MA50"], mode="lines", name="MA 50-Day "
            )
        )
        fig1.add_trace(
            go.Scatter(
        ...
        fig1.update_layout(
            title=f"{etf} Price trend and Moving Avg.",
            xaxis_title="Date",
            yaxis_title="Price",
        )
